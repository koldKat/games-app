...
- With no platform filter, copies of the same canonical game are collected into one card. When more than one copy exists, open the platform badge and choose a copy from its themed dropdown. IGDB-linked copies can group even when their edition names differ; older or unmatched copies fall back to normalized titles. Applying a platform filter intentionally expands the matching records into separate cards.
- Selecting **Multiple platforms** keeps matching titles grouped and shows only games recorded on at least two distinct platforms. Two copies on the same platform do not qualify.
...
- Live process health (heap, RSS/CPU, cumulative traffic in/out, app age, and session uptime, refreshed each second) plus whole-database game, account, cover, description, PEGI, HowLongToBeat, rating, catalog, favorite, platform, ownership, media-format, and play-state summaries (refreshed each minute). The compact dashboard stacks related cards by library, community catalog, metadata coverage, runtime, and process/storage health. App age begins with the earliest user or game record. Every positive clean-stop or heartbeat gap is counted as downtime; the fifteen-second threshold determines only whether the next process begins a new uptime session.
- Account inspection, last-active and inactivity age, approximate country/city from recent authenticated activity, session revocation, manual lock/unlock, and typed-confirmation account deletion. Accounts inactive for 31 days or longer are tucked behind a compact reveal row, matching Gamebooks, while recent accounts remain immediately visible. Location is resolved locally from the connection address with the same offline GeoIP database used by Gamebooks; only the country code and city are stored, never the IP address. The Accounts table uses the available desktop width without an unnecessary horizontal scrollbar, while narrow screens retain deliberate horizontal access. Locking signs the account out everywhere; automatic temporary locks follow five incorrect passwords. The server's configured owner account cannot be locked or deleted. Deleting another account also deletes its avatar, games, sessions, integration settings, and saved interface preferences.
