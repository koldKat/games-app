...
Progression is split into three focused server modules. `progression-policy.js` is the dependency-free definition of all 36 XP event defaults, the exact Gamebooks triangular level curve (`1000 × level × (level + 1) / 2`), and collector titles. `progression-store.js` owns the SQLite tables, configurable amounts, atomic idempotency key `(user_id, event, ref)`, and named per-account migration markers. `progression-service.js` maps a complete game record to eligible one-time awards and evaluates account milestones and collection achievements.

`user_progression` stores the account XP total and original one-time backfill marker. `progression_events` stores every granted award with its stable reference; uniqueness guarantees that repeating a qualifying state cannot grant the same event twice. `progression_backfills` records independently deployable, per-account historical reconciliations. `progression_config` is localhost-admin-editable, automatically seeds every policy event, and changes future awards only. No interval, login, uptime, or idle path grants XP. Game create/update, enrichment SSE paths, Kat·a·log additions, forum posts, and a first avatar set all feed the same service. Forum replies award the author and, once per thread, award its owner when another account engages.

Collection-achievement evaluation uses owned, non-hidden records. It groups copies by canonical game ID with a normalized-title fallback. Two distinct platforms award `multiplatform_collector` once per title; physical and digital copies on the same title/platform award `format_double_dip` once per release; 25 distinct titles on a platform award `platform_specialist` once per platform. Normal saves evaluate the current collection, while imports defer the collection-wide pass until their final batch. A lightweight projection avoids hydrating metadata during the one-time startup backfill. The named migration marker is written only after the account scan completes, while the event ledger keeps a retry after interruption idempotent. Backfilled awards use the configured values and are published through the normal progression and level-up path.

Progression emits a `progression-updated` SSE event only when XP changes. `public/js/progression-ui.js` hydrates its animation baseline from the authenticated summary before connecting SSE, so either the live event or the save response animates from the XP already on screen and the duplicate is ignored. Signal independently derives any historical level crossings from that immutable XP ledger, preserving the original award timestamp and never duplicating an already recorded level.
...
| `canonical_game_id`, `canonical_release_id` | Nullable links to the shared title-level identity and exact platform release |
...
| Method | Route | Purpose |
|---|---|---|
| GET | `/api/games` | Return one filtered, grouped library page plus total/page metadata |
...
| GET | `/api/public/user/:username` | Opt-in public collector identity, progression summary, aggregate stats, and every owned, non-hidden platform |
