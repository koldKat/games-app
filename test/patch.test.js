...
test('Patch and Ping remain private, modular, and locally triaged', () => {
  ...
  assert.match(routes, /emailOperatorPatch/); assert.match(read('server/mailer.js'), /function sendOperatorPatch/); assert.match(read('server/email-templates.js'), /OPEN GAME KAT·A·LOG/);
