...
test('HLTB lookup targets the current endpoint and normalizes title matching', () => {
  ...
  assert.match(provider, /if \(!auth\.token\) throw new Error/);
  assert.match(provider, /'x-auth-token': String\(auth\.token\)/);
  assert.doesNotMatch(provider, /x-hp-key|x-hp-val|dynamicKey|dynamicValue/);
  assert.match(provider, /year: '', modifier: ''/);
