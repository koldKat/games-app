...
function fixture() {
  ...
  return createCatalogueRoutes({
    ...
    auth: { authenticate: () => null, refreshSessionCookie: () => null },
    events: { publish() {} },
  });
}

test('public catalogue routes render before account authentication', async () => {
  const routes = fixture(); const output = response();
  const handled = await routes.handle({ method: 'GET' }, output, new URL('https://gamekat.net/catalogue'));
