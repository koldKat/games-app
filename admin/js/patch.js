...
function messageMarkup(thread, message) {
  ...
  return `<article class="admin-patch-message admin-patch-message--${message.sender}"><span>${sender}</span><p>${esc(message.body).replace(/\n/g, '<br>')}</p><time>${esc(date(message.createdAt))}</time></article>`;
}
function patchCardMarkup(thread) {
  const last = thread.messages?.at(-1);
  const messages = (thread.messages || []).map(message => messageMarkup(thread, message)).join('');
  const count = thread.messages?.length || 0;
  return `<article class="admin-patch-card${thread.adminUnread ? ' unread' : ''}" data-admin-patch="${thread.id}">
  <header><button class="admin-patch-toggle" type="button" aria-expanded="false"><span><b>${esc(kinds[thread.kind] || 'PATCH')} // ${esc(thread.username || 'ANONYMOUS')}</b><small>${esc(last?.body?.slice(0, 130) || '')}</small></span><time>${esc(date(last?.createdAt || thread.createdAt))}</time></button></header>
  <div class="admin-patch-body" hidden>
    <div class="admin-patch-contact">${thread.email ? esc(thread.email) : 'No email supplied'} // ${count} message${count === 1 ? '' : 's'}</div>
    <div class="admin-patch-messages">${messages}</div>
    <form class="admin-patch-reply"><textarea maxlength="4000" placeholder="Reply through Ping"></textarea><footer><button class="button primary" type="submit">Send reply</button><button class="button danger" type="button" data-admin-patch-delete>Remove from queue</button></footer></form>
  </div>
</article>`;
}
...
export async function loadPatch() {
  ...
  try {
    ...
    host.innerHTML = threads.length ? threads.map(patchCardMarkup).join('') : '<p class="panel-note">No Patch threads yet.</p>';
