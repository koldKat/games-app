...
function createProgressionStore(db) {
  ...
  function configuredAmounts() {
    if (!amountCache) amountCache = new Map(configRows.all().map(row => [row.event, row.amount]));
    return amountCache;
  }
  ...
  const txAward = db.transaction((userId, event, ref) => {
    ...
    ensure.run(userId); const before = progressForXp(getXp.get(userId).xp); const amount = Number(configuredAmounts().get(event) ?? XP_EVENTS[event].amount);
