...
function pageShell({ title, description, canonical, content, structuredData, user = null, socialImage = `${SITE_URL}/social-preview.png` }) {
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
function renderCatalogue({ result, platforms, query = '', platform = '', user = null }) {
  ...
  return pageShell({
    title: 'Public Kat·a·log | Game Kat·a·log', description, canonical: `${SITE_URL}/catalogue`, user,
    structuredData: { '@context': 'https://schema.org', '@type': 'CollectionPage', name: 'Game Kat·a·log Public Kat·a·log', url: `${SITE_URL}/catalogue`, numberOfItems: result.total },
    content: `<main class="catalogue-main"><section class="catalogue-hero"><p>SHARED // GROWING</p><h1>The public Kat·a·log</h1><span>Discover enriched releases and add them to your private library without entering everything again.</span></section>
      <form class="catalogue-search" action="/catalogue" method="get"><label><span>Search Kat·a·log</span><input type="search" name="q" value="${escapeHtml(query)}" placeholder="Title, publisher, or platform" maxlength="120"></label><label><span>Platform</span><select name="platform"><option value="">All platforms</option>${platformOptions}</select></label><button type="submit">Search</button></form>
      <div class="catalogue-results"><div class="catalogue-result-head"><strong>${result.total.toLocaleString()} public release${result.total === 1 ? '' : 's'}</strong>${query || platform ? `<a href="/catalogue">Clear search</a>` : ''}</div>
      <section class="catalogue-grid">${cards || '<div class="catalogue-empty"><strong>No matching releases.</strong><span>The Kat·a·log grows as members enrich their private libraries.</span></div>'}</section>${pagination}</div></main>`,
  });
}
...
function renderGame({ entry, user = null }) {
  const description = `${entry.title} for ${entry.platform}: PEGI ${entry.pegi || 'unrated'} information, cover art, publisher details, and HowLongToBeat estimates.`;
  ...
  return pageShell({
    title: `${entry.title} (${entry.platform}) | Game Kat·a·log`, description, canonical, user,
    socialImage: `${SITE_URL}${entry.coverUrl}`,
...
function renderNotFound() {
  return pageShell({
    ...
    canonical: `${SITE_URL}/catalogue`, structuredData: { '@context': 'https://schema.org', '@type': 'WebPage', name: 'Game not found' },
    content: '<main class="catalogue-main"><div class="catalogue-empty"><strong>That release is not public.</strong><a href="/catalogue">Browse the Kat·a·log</a></div></main>',
  });
}
...
function sitemapXml(entries, today = new Date().toISOString().slice(0, 10)) {
  const urls = [
    ...
    [`${SITE_URL}/catalogue`, 'daily', '0.9', today],
